...
<h1 style="font-family:'华文楷体';margin-left:8%;">处方药接口测试</h1>

<div class="parms_div">
    <span style="font-family:'华文楷体';">接口名称：</span>
    <select id="apipick" autocomplete="off">
        <option value="选择测试接口"  >选择测试接口</option>
    ...
    <div id='<?php echo $i ?>' style="display:none;" class="parms">
        ...
        <form action="<?php echo $key ?>">
            <table>
                ...
                <?php
 if (is_string($value) && strpos($value, '{') !== false && strpos($value, '}') !== false){ $letters = array('{', '}'); $fruit = array(' ', ' '); $model = strtoupper(trim(str_replace($letters, $fruit, $value))); if (isset($data['model'][$model])) { echo '<tr><td class="align">'.$model."：</td><td>"; foreach ($data['model'][$model] as $k => $v) { echo trim(str_replace('!', '',$k)).':&nbsp;<input type="text" style="font-family:华文楷体;" name="'.trim(str_replace('!', '',$k)).'" placeholder="'.$v.'" ><br/>'; } echo '</td></tr>'; } }else{ if (is_array($value)){ $value = implode(',', $value); } $pic_arr = array("images_list"); if(in_array(trim(str_replace('!', '',$key)),$pic_arr)){ echo '<tr><td class="align">'.trim(str_replace('!', '',$key)).'：</td><td><input type="file" style="font-family:华文楷体;" id="'.trim(str_replace('!', '',$key)).'" name="'.trim(str_replace('!', '',$key)).'[]" placeholder="'.$value.'" multiple="multiple"></td></tr>'; }else{ echo '<tr><td class="align">'.trim(str_replace('!', '',$key)).'：</td><td><input type="text" style="font-family:华文楷体;" name="'.trim(str_replace('!', '',$key)).'" placeholder="'.$value.'" ></td></tr>'; } } ?>
...
<script type="text/javascript" charset="utf-8">
    $(function(){
        ...
        $("#send").click(function(){
            ...
            //要上传图片的接口地址数组
            var arrlist = new Array("/CurrentStatus/create","/order/photo_order");
